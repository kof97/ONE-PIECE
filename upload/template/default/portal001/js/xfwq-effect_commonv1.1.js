...
(function($){
    dk_slideplayer = function(object,config){
        ...
        if(this.config.fontsize == "20px"){
            this.size = "20px";this.height = "23px";this.right = "6px";this.bottom = "15px";
        }else{
            this.size = "18px";this.height = "21px";this.right = "6px";this.bottom = "10px";
        }
        ...
        this.factory = function(){
            ...
            $(this.obj + " .dkTitleBg").css({position:"absolute",margin:"0",padding:"0",zIndex:"1",bottom:"0",left:"0",width:"100%",height:"50px",background:"#000",opacity:"0.4",overflow:"hidden"})
            //插入标题
            $(this.obj).prepend("<div class='dkTitle'></div>");
            $(this.obj + " p").each(function(i){			
                $(this).appendTo($(_this.obj + " .dkTitle")).css({position:"absolute",margin:"0",padding:"0",zIndex:"2",bottom:"10px",left:"0",width:"100%",height:_this.height,lineHeight:_this.height,textIndent:"5px",textDecoration:"none",fontSize:_this.size,color:"#FFFFFF",background:"none",opacity:"1",overflow:"hidden"});
                if(i!= 0){$(this).hide()}
            });
            this.slide();
            this.addhover();
